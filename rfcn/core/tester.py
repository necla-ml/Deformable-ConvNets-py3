...
from .module import MutableModule    # py3
...
from dcn.utils import image
from dcn.utils.PrefetchingIter import PrefetchingIter
...
def pred_eval(predictor, test_data, imdb, cfg, vis=False, thresh=1e-3, logger=None, ignore_cache=True):
    ...
    for im_info, data_batch in test_data:
        ...
        net_time += t2
        post_time += t3
        print('testing {}/{} data {:.4f}s net {:.4f}s post {:.4f}s'.format(idx, imdb.num_images, data_time / idx * test_data.batch_size, net_time / idx * test_data.batch_size,
